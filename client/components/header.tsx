...
export default function Header() {
  ...
  const phone = companyData?.sectionOne?.phone1?.replace(/\D/g, "");
  const phone2 = companyData?.sectionOne?.phone2?.replace(/\D/g, "");

  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/coursedetails`)
        const companyDetailsRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/companydetails`)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <Link href="/" className="flex items-center">
            <Image src={companyData?.sectionOne?.headerLogoUrl || "/vijanaar-logo.png"} alt="Vijanaar" width={170} height={75} className="h-16 w-auto" />
          </Link>
          ...
          <div className="flex items-center space-x-4 mr-[20px] xl:mr-0">
            <div className="hidden lg:flex items-center text-gray-700 transition">
              <svg className="w-5 h-5 mr-2 text-green-500 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
              ...
              <div className="flex flex-col leading-tight">
                <a href={`https://wa.me/${phone}`} target="_blank" rel="noopener noreferrer" className="font-medium hover:text-green-600 transition">{companyData?.sectionOne?.phone1 || "+91 99634-90606"}</a>
                {companyData?.sectionOne?.phone2 && (
                  <a href={`https://wa.me/${phone2}`} target="_blank" rel="noopener noreferrer" className="font-medium hover:text-green-600 transition">{companyData.sectionOne.phone2}</a>
                )}
              </div>
            </div>
