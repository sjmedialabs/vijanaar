    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/training-programs`
        );
        if (res.data?.bannerSection) {
          setInputData(res.data.bannerSection);
        }
      } catch (err) {
        console.error("Error fetching Training Program page:", err);
        setError(true);
      }
    };
    fetchData();
...
const saveHandler = async () => {
  ...
  try {
    ...
    // Send the correct payload matching backend schema
    const payload = {
      bannerSection: {
    ...
    console.error("Error updating Training Program page:", err);
    setError(true);
